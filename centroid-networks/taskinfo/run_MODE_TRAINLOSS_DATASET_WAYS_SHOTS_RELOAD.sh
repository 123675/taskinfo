...
PYTHONPATH=.. python ../scripts/train/few_shot/run_train.py --data.dataset ${DATASET} --data.root $HOME/data/${DATASET} --model.model_name "pairwise_conv" --log.exp_dir results-${DATASET}-${WAYS}way-${SHOTS}shot/${DATASET}-${WAYS}way-${SHOTS}shot-$TRAINLOSS --train-loss "${ACTUALTRAINLOSS}" --centroid-loss 0 --validate-interval 5 --iterations 100000 --data.way ${WAYS} --data.test_way ${WAYS} --train.learning_rate 0.0001 --data.cuda ${RELOAD_CMD}
